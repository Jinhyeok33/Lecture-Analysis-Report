...
class SummaryConceptClarityScores(FloatScoreBase):
    concept_definition: float = Field(ge=1.0, le=5.0)
    analogy_example_usage: float = Field(ge=1.0, le=5.0)
    prerequisite_check: float = Field(ge=1.0, le=5.0)

class SummaryPracticeLinkageScores(FloatScoreBase):
    example_appropriateness: float = Field(ge=1.0, le=5.0)
    practice_transition: float = Field(ge=1.0, le=5.0)
    error_handling: float = Field(ge=1.0, le=5.0)

class SummaryInteractionScores(FloatScoreBase):
    participation_induction: float = Field(ge=1.0, le=5.0)
    question_response_sufficiency: float = Field(ge=1.0, le=5.0)
