    ...
    def _extract_jargon_candidates(self, scripts):
        ...
        for text in scripts:
            ...
            for w in words:
                ...
                pattern = r'(해라|이라|받자|하자|해보자|하겠|하|할까|라|인데|해|시키|이죠|죠|하시|해봐|이든|들|주래|이나|나|해보시라|이란|했어요|하세요|고요|거든|인지|지|와|이면서|에요|이었어요|이라든지|입니다|습니다|합니다|이라서|이니까|이잖아|에다가|에다|에서|부터|까지|마다|로서|로써|보다|처럼|라는|다는|랑|이랑|라서|니까|잖아|은|는|이|가|을|를|에|로|으로|도|만|의|다|고|며|면|야|쯤|용|할|단|인|니)$'
