...
from src.llm_engine.core.exceptions import (
    CotMismatchError,
    HallucinationError,
    LanguageViolationError,
    NonRetryableAPIError,
    RefusalError,
    TruncatedResponseError,
)
from src.llm_engine.core.metrics import get_metrics
from src.llm_engine.core.ports import (
    AggregatorPromptBuilder,
...
def _flatten_scores(scores: ChunkScores) -> dict[str, int | None]:
    ...
    for category_scores in scores.model_dump().values():
        if isinstance(category_scores, dict):
            flat.update(category_scores)
    return flat
...
def _default_prompt_builder() -> AggregatorPromptBuilder:
    from src.llm_engine.application.prompts import build_aggregator_refine_prompt
...
class BaseLLMAdapter(ILLMProvider, abc.ABC):
    ...
    def _compute_reliability(
        ...
        scored = [(item, value) for item, value in flat.items() if value is not None]
    ...
    def _build_success_result(
        ...
        payload = parsed.final_output
        score_map = _flatten_scores(payload.scores)
        cleaned_evidence = [
            evidence
            for evidence in detail.passed
            if score_map.get(evidence.item) is not None and score_map[evidence.item] != 3
        ]
    ...
    def analyze_chunk(self, chunk_data: ChunkMetadata) -> ChunkResult:
        ...
        parsed, retry_count, usage, detail, hallucination_retries = self._request_structured(chunk_data)
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        reliability = self._compute_reliability(detail, parsed.final_output.scores, hallucination_retries)
    ...
    async def analyze_chunk_async(self, chunk_data: ChunkMetadata) -> ChunkResult:
        started = time.perf_counter()
        parsed, retry_count, usage, detail, hallucination_retries = await self._request_structured_async(chunk_data)
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        reliability = self._compute_reliability(detail, parsed.final_output.scores, hallucination_retries)
        result = self._build_success_result(chunk_data, parsed, detail, retry_count, usage, reliability)
        result.elapsed_ms = elapsed_ms
    ...
    def aggregate_results(
        ...
        user_content = self._build_aggregator_prompt(items[:AGGREGATOR_MAX_ITEMS], label, scores_context, trends)
        ...
        for attempt in range(1, max_aggregate_retries + 1):
            ...
            except (RefusalError, NonRetryableAPIError) as exc:
                logger.error("LLM 통합 요약 복구 불가 오류 (%s): %s", type(exc).__name__, exc)
                raise
        ...
        logger.warning(
            "LLM 통합 요약 %d회 시도 후 실패; 어휘 기반 중복 제거로 대체. 마지막 오류: %s",
            max_aggregate_retries,
            last_error,
        )
    ...
    def _check_language(parsed: LLMInternalResponse) -> None:
        ...
        texts = [
            ...
            *(payload.issues or []),
            *(evidence.reason for evidence in (payload.evidence or [])),
            *(cot.anchor for cot in parsed.cot),
        ]
        for text in texts:
            if _contains_english_sentences(text):
                raise LanguageViolationError(f"언어 위반 감지: 응답에 영어 문장 혼입 — '{text[:80]}...'")

    @staticmethod
    def _validate_cot_consistency(parsed: LLMInternalResponse) -> None:
        cot_scores = {evaluation.item: evaluation.score for evaluation in parsed.cot}
        final_flat = _flatten_scores(parsed.final_output.scores)
        mismatches = [
            f"{item}: CoT={cot_value} ≠ output={final_flat.get(item)}"
            for item, cot_value in cot_scores.items()
            if not (cot_value is None and final_flat.get(item) is None) and cot_value != final_flat.get(item)
        ]
        if mismatches:
            raise CotMismatchError(
                f"CoT 정합성 위반 감지 ({len(mismatches)}건): " + "; ".join(mismatches[:5])
            )
    ...
    def _handle_retry_error(self, exc: Exception, chunk_id: int, attempt: int) -> None:
        ...
        if self._should_not_retry(exc):
            logger.error("chunk_id=%02d attempt=%d %s retryable=False: %s", chunk_id, attempt, type(exc).__name__, exc)
            raise NonRetryableAPIError(f"재시도 불가 오류: {exc}") from exc

        logger.warning("chunk_id=%02d attempt=%d %s: %s", chunk_id, attempt, type(exc).__name__, exc)
    ...
    async def _request_structured_async(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                logger.warning("chunk_id=%02d attempt=%d CancelledError — 재전파", chunk_data.chunk_id, attempt)
                raise
            except Exception as exc:
                if isinstance(exc, HallucinationError):
                    hallucination_retries += 1
    ...
    def _deduplicate_ranked(self, values: List[str]) -> List[str]:
        ...
        for candidate, _ in ranked:
            if _HAS_RAPIDFUZZ:
                is_duplicate = any(_rfuzz.ratio(candidate, existing) >= threshold_100 for existing in result)
            else:
                is_duplicate = any(
                    _SequenceMatcher(None, candidate, existing).ratio() * 100 >= threshold_100
